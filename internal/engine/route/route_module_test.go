...
func TestRegisterModuleRoutes_ReservedStorage(t *testing.T) {
	table := New()
	err := RegisterModuleRoutes(table, "storage", "domain", []ExplicitRoute{
		{Method: "GET", Path: "/"},
	})
	if err == nil {
		t.Fatal("want error: expanded path's first segment is \"storage\"")
	}
}
...
func TestRegisterModuleRoutes_ReservedUsers(t *testing.T) {
	table := New()
	err := RegisterModuleRoutes(table, "users", "domain", []ExplicitRoute{
		{Method: "POST", Path: "/invite"},
	})
	if err == nil {
		t.Fatal("want error: expanded path's first segment is \"users\"")
	}
}
