...
func newORMTestModuleContext(tenantSlug string, modelDecls []model.ModelDeclaration) *ModuleContext {
	return NewModuleContext("req-1", "testmodule", "user-1", "contact-1", []string{"admin"}, "tenant-id-1", tenantSlug, "trace-1", abi.CapDBRead, nil, modelDecls, nil)
}
...
func TestHostORM_Search_CapabilityDenied(t *testing.T) {
	...
	r := newHostDBTestRuntime(t, primaryDB, 10)
	mc := NewModuleContext("req-1", "testmodule", "user-1", "contact-1", []string{"admin"}, "tenant-id-1", slug, "trace-1", abi.CapabilitySet(0), nil, []model.ModelDeclaration{widgetModelDecl()}, nil)
	inst := newHostORMCaller(t, ctx, r, mc)
...
func TestHostORM_Search_RespectsRLS(t *testing.T) {
	...
	mc := NewModuleContext("req-1", "testmodule", "user-1", repID, []string{"admin"}, "tenant-id-1", slug, "trace-1", abi.CapDBRead, nil, []model.ModelDeclaration{widgetModel}, nil)
	inst := newHostORMCaller(t, ctx, r, mc)
