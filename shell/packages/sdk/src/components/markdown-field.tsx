...
export interface MarkdownFieldProps {
  ...
  // contentEditable isn't natively labelable (goerp#698) — external label
  // target, matching RichTextField/CodeField.
  ariaLabelledBy?: string | undefined;
  // @tiptap/markdown's serialized Markdown source, not HTML.
  value: string;
  onChange: (value: string) => void;
  placeholder?: string | undefined;
  // Approximate min-height, matching RichTextField/CodeField's `rows` prop.
  rows?: number | undefined;
  error?: string | undefined;
  disabled?: boolean | undefined;
}

// underline: false — CommonMark/GFM has no native underline syntax, so
// keeping RichTextField's Underline button would either embed raw HTML <u>
// in otherwise-plain Markdown source or silently lose the formatting on
// serialization. link.openOnClick: false matches RichTextField's own
// reasoning — this editor's Link button needs a plain click on link text to
...
export function MarkdownField({
  ...
  // clobbering in-progress typing — only fires when the editor's own
  // Markdown actually diverges from the prop. `emitUpdate: false` is
  // required here too, for the same reason as setEditable above.
  //
  // Compares against `value` re-parsed and re-serialized through the same
  // MarkdownManager, not the raw prop: @tiptap/markdown's serializer
  // unconditionally backslash-escapes \`*_[]~ in plain text, so a value
  // containing any of those characters never equals editor.getMarkdown()
  // even when nothing has actually changed — comparing raw would reset the
  // document (and the user's selection) on every render carrying such a
  // value, including a same-content re-fetch mid-edit.
  useEffect(() => {
    if (!editor?.markdown) return;
    const canonicalValue = editor.markdown.serialize(editor.markdown.parse(value));
    if (editor.getMarkdown() !== canonicalValue) {
      ...
      <div className={fieldInputClassName(error !== undefined, "wrapper", "sans")}>
        ...
        <div className="relative">
          {placeholder !== undefined && value === "" && (
            <span aria-hidden="true" className="pointer-events-none absolute top-2 left-0 text-sm text-text-secondary">
              {placeholder}
            </span>
          )}
